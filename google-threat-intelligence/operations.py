...
class GoogleThreatIntelligence(object):
...
def download_file(config, params):
    ...
    try:
        ...
        response = gti.make_rest_call(endpoint, 'GET')
...
def create_zip_file(config, params):
    ...
    try:
        ...
        response = gti.make_rest_call(endpoint, 'POST', data=json.dumps(payload))
...
def search_intelligence(config, params):
    ...
    try:
        ...
        payload = check_payload(payload)
        response = gti.make_rest_call(endpoint, 'GET', params=payload)
        return response
    except Exception as err:
        logger.exception("{0}".format(str(err)))
        raise ConnectorError("{0}".format(str(err)))
...
def update_livehunt_ruleset(config, params):
    ...
    try:
        ...
        payload = check_payload(payload)
        response = gti.make_rest_call(endpoint, 'PATCH', data=json.dumps(payload))
        return response
    except Exception as err:
        logger.exception("{0}".format(str(err)))
        raise ConnectorError("{0}".format(str(err)))
...
def delete_livehunt_ruleset(config, params):
    gti = GoogleThreatIntelligence(config)
    endpoint = 'intelligence/hunting_rulesets/{0}'.format(params.get('id'))
    response = gti.make_rest_call(endpoint, 'DELETE')
    if response:
        return response
    else:
...
def create_retrohunt_job(config, params):
    ...
    if 'T' in start_time:
        start_time = int(convert_datetime_to_unix_time(start_time))
    end_time = params.get('end_time')
    if 'T' in end_time:
        end_time = int(convert_datetime_to_unix_time(end_time))
...
def abort_retrohunt_job(config, params):
    ...
    try:
        response = gti.make_rest_call(endpoint, 'POST')
        return response
    except Exception as err:
        logger.exception("{0}".format(str(err)))
        raise ConnectorError("{0}".format(str(err)))
...
def submit_file(config, params):
    try:
        ...
        if file_data:
            ...
            res = gti.make_rest_call(endpoint, 'POST', files=files)
            if res.get('error'):
...
def scan_url(config, params):
    try:
        ...
        response = gti.make_rest_call(endpoint, 'POST', data=params)
        if response.get('error'):
            return response.get('error')
        return response.get('data')
    except Exception as err:
...
def get_widget_rendering_url(config, params):
    try:
        gti = GoogleThreatIntelligence(config)
        if params.get('theme'):
            params['theme'] = params.get('theme').lower()
        endpoint = 'gtiwidget'
        response = gti.make_rest_call(endpoint, 'GET', params=params)
        if response.get('error'):
            return response.get('error')
        return response
    except Exception as err:
...
def get_entities_list(config, params):
    try:
        gti = GoogleThreatIntelligence(config)
        payload = {
        ...
        if params.get('collection_type'):
            query_parameter = "collection_type:{0} ".format(COLLECTION_TYPE.get(params.get('collection_type')))
        if params.get('creation_date'):
            creation_date = params.get('creation_date')
            if 'T' in creation_date:
                creation_date = str(convert_datetime_to_unix_time(creation_date)) + "+"
                query_parameter += f"creation_date:{creation_date} last_modification_date:{creation_date} "
            else:
                creation_date = str(creation_date) + "+"
                query_parameter += f"creation_date:{creation_date} last_modification_date:{creation_date} "
        if params.get('origin'):
            query_parameter = query_parameter + "origin:{0}".format(params.get('origin'))
        if query_parameter:
            query_parameter = urllib.parse.quote(query_parameter.strip())
            endpoint = 'collections?filter=' + query_parameter
        else:
...
def get_entities_details(config, params):
    try:
        gti = GoogleThreatIntelligence(config)
        endpoint = 'collections/{0}'.format(params.get('id'))
        response = gti.make_rest_call(endpoint, 'GET')
        if response.get('error'):
            return response.get('error')
        return response
...
def get_mitre_tactics_and_techniques(config, params):
    try:
        gti = GoogleThreatIntelligence(config)
        endpoint = 'collections/{0}/mitre_tree'.format(params.get('id'))
        response = gti.make_rest_call(endpoint, 'GET')
        if response.get('error'):
            return response.get('error')
        return response
...
def _check_health(config):
    try:
        gti = GoogleThreatIntelligence(config)
        endpoint = gti.url + "users/{0}".format(config.get('api_key'))
        headers = {
...
operations = {
    ...
    'get_entities_details': get_entities_details,
    'get_mitre_tactics_and_techniques': get_mitre_tactics_and_techniques,
